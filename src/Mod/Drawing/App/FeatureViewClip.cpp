...
PROPERTY_SOURCE(Drawing::FeatureViewClip, Drawing::FeatureView)
...
FeatureViewClip::~FeatureViewClip()
{
}

void FeatureViewClip::onChanged(const App::Property* prop)
{
    if (prop == &Height ||
        prop == &Width  ||
        prop == &ShowFrame ||
...
short FeatureViewClip::mustExecute() const
{
    if (Views.isTouched()) {
        return 1;
    } else {
...
std::vector<std::string> FeatureViewClip::getChildViewNames()
{
...
namespace App {
/// @cond DOXERR
PROPERTY_SOURCE_TEMPLATE(Drawing::FeatureViewClipPython, Drawing::FeatureViewClip)
template<> const char* Drawing::FeatureViewClipPython::getViewProviderName(void) const {
    return "DrawingGui::ViewProviderDrawingClip";
}
/// @endcond

// explicit template instantiation
template class DrawingExport FeaturePythonT<Drawing::FeatureViewClip>;
}
