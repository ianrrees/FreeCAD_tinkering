...
#include "GIViewBase.h"

using namespace TechDraw;

GIViewBase::GIViewBase(const QPoint &pos, QGraphicsScene *scene)
    : QGraphicsItemGroup(),
      locked(false),
      m_innerView(false)
{
...
const char * GIViewBase::getViewName() const
{
    return viewName.c_str();
}

void GIViewBase::setViewFeature(TechDraw::DrawView *obj)
{
    if(obj == 0)
        return;
...
DrawView * GIViewBase::getViewObject() const
{
     return viewObj;
}

void GIViewBase::setPosition(qreal x, qreal y)
{
    if (!isInnerView()) {
...
double GIViewBase::getYInClip(double y)
{
...
void GIViewBase::alignTo(QGraphicsItem *item, const QString &alignment)
{
    alignHash.clear();
    alignHash.insert(alignment, item);
}

void GIViewBase::updateView(bool update)
{
    if (update ||
...
void GIViewBase::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    QGraphicsItemGroup::paint(painter, option, widget);
}

QVariant GIViewBase::itemChange(GraphicsItemChange change, const QVariant &value)
{
